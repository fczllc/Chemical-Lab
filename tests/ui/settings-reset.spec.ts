...
test.describe('Settings data reset flow', () => {
  test('shows requested settings layout and preserves settings while clearing learning data', async ({ page }) => {
    ...
    expect(catMetrics.borderRadius).toContain('50%');
    ...
    expectSeededState(beforeCancel);
    ...
    expect(afterConfirm.storedEnvelope.version).toBe('v2');
    ...
    expect(afterReload.storedEnvelope.version).toBe('v2');
